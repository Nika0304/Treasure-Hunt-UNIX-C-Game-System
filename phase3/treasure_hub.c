...
void commandFile(char* msg)
{
    ...
    char buffer[2048];
    ssize_t bytesRead = read(monitor_pipe[0], buffer, sizeof(buffer) - 1); // Read from the pipe.
    if (bytesRead > 0) //daca a reusit citirea, afisam
    {
        buffer[bytesRead] = '\0';
        printf("%s", buffer);
    } 
    ...
    {
        perror("Failed to read from monitor pipe");
    }
}
...
void handle_signal1(int sig)
{
    ...
    else if(strcmp(aux, "calculate_score") == 0)
    {
...
void handler_sigchld(int sig)
{
    ...
    // Așteaptă TOATE procesele copil care au terminat
    while ((pid = waitpid(-1, &status, WNOHANG)) > 0)
    {
        if (pid == monitor_pid) // doar dacă monitorul a murit
        {
...
void start_monitor()
{
    ...
    else if (pid == 0)
    {
        ...
        dup2(monitor_pipe[1], 1); // redirectioneaza stdout catre capatul de scriere al pipe-ului
        close(monitor_pipe[1]);// il inchide 
    ...
    {
        // Parent process
        close(monitor_pipe[1]); //close the write end
...
void calculate_score() 
{
    ...
    while ((dname = readdir(d)) != NULL) 
    {
        if (strcmp(dname->d_name, ".") == 0 || strcmp(dname->d_name, "..") == 0) continue;
            if (dname->d_type == DT_DIR)
            {
                ...
                // Child
                    ...
                    execlp("./score_calculator", "score_calculator", dname->d_name, NULL);
                ...
                // Parent
                    ...
                    close(pipefd[0]);
                    waitpid(pid, NULL, 0);//asteapta terminarea unui proces copil
                }
            }
    }
